...
/* Queryy 7: Is there a significant difference in Breaking Ball (Slider/Curveball) 
...
/* Query 8: Does relying heavily on Fastballs (Four-Seam/Sinker) correlate with 
...
/* Query 9: How does the usage of offspeed pitches (Changeup/Splitter) relate to 
injury outcomes? */
...
/* Query 10: Which pitch types are used on average more than 20% of the time 
...
/* Query 11: Who are the pitchers that have had TJS, and what is the pitch 
...
/* Query 12: Which pitch type has the highest average usage among the TJS group? */
...
/* Query 13: Which pitch type has the highest average usage among the TJS group?*/
...
/* Query 14: Average Spin rate amoung Healthy and Injured players */
...
/* Query 15: This query groups pitch types by mechanical stress (High-Torque Breaking vs. Low-Tension Off-speed) to demonstrate that while fastballs are the foundation of all repertoires, the frequency of breaking balls is the primary driver of TJS prevalence */

WITH InjuredPitchers AS (
    SELECT COUNT(DISTINCT Name) as TotalInjured FROM Injuries WHERE TJS = 1
),
CategorizedUsage AS (
    SELECT 
        PU.PitcherName,
        CASE 
            WHEN PU.PitchType IN ('FF', 'SI') THEN 'Fastball'
            WHEN PU.PitchType IN ('SL', 'CU', 'KC', 'ST', 'FC') THEN 'Breaking Ball'
            WHEN PU.PitchType IN ('CH', 'FS') THEN 'Off-speed'
            ELSE 'Other'
        END AS Category
    FROM PitchUsage PU
    JOIN Injuries I ON PU.PitcherName = I.Name
    WHERE I.TJS = 1 AND PU.UsagePercentage >= 20.0 
)
SELECT 
    Category,
    (COUNT(DISTINCT PitcherName) * 100.0 / (SELECT TotalInjured FROM InjuredPitchers)) AS Percent_of_Injured_Group
FROM CategorizedUsage
